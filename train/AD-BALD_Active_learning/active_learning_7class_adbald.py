...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, epochs, patience=10):
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            best_model_wts = copy.deepcopy(model.state_dict())
            torch.save(model.state_dict(), '/home/work/colonoscopy_data/dataset/active_learning_result/valyes/class7/semiact_best_model_7class.pth')
...
def adjust_threshold(current_accuracy, previous_accuracy, threshold, adjustment_factor=0.1, max_threshold=0.9, min_threshold=0.5, change_factor=0.7):
    # 정확도 변화 계산
...
def predict_prob_dropout_split(model, data_loader, n_drop, device='cuda'):
    model.train()
    all_probs = []
...
def bald_query_with_threshold(model, samples_loader, n_drop=15, uncertainty_threshold=0.3, device='cuda'):
    ...
    print(f"Number of data points selected: {len(query_indices)}")
...
# 다중 클래스에 대한 메트릭 계산 함수
def calculate_metrics_multi(loader, model, num_classes):
    model.eval()
    all_preds = []
    all_labels = []
    all_probs = []
    ...
    with torch.no_grad():
        for data in loader:
            images, labels, _ = data  # Ignore the third value (paths)
            images, labels = images.to(device), labels.to(device)
            outputs = model(images)
            _, predicted = torch.max(outputs.data, 1)
    ...
    # F1-score
    f1 = f1_score(all_labels, all_preds, average='macro')  # Macro average for F1-score

    # AUROC (One-vs-Rest)
    try:
        auroc = roc_auc_score(all_labels, all_probs, multi_class='ovr')
    except ValueError:
        auroc = float('nan')  # In case there's an issue with positive samples for some classes
    ...
    return avg_sensitivity, avg_specificity, f1, auroc
...
def evaluate_model_with_cm(model, test_loader, device, criterion, round_num, categories):
    ...
    # Confusion matrix
    cm = confusion_matrix(all_labels, all_preds)
    plot_confusion_matrix(cm, categories, round_num, accuracy, f'7class_confusion_matrix_round_{round_num}.png')
    
    return accuracy, avg_loss, cm

train_dir = '/home/work/colonoscopy_data/dataset/class7/semi_active_learning/train/train7/'
val_dir='/home/work/colonoscopy_data/dataset/class7/semi_active_learning/train/val7/'
test_dir = '/home/work/colonoscopy_data/dataset/class7/semi_active_learning/test'
unlabeled_dir = '/home/work/colonoscopy_data/dataset/class2/semi_active_learning/train/unlabeled/'
categories = ['bad_light', 'blurry', 'bubble', 'obstacles', 'tool', 'wall','informative']
...
unlabeled_loader = DataLoader(unlabeled_dataset, batch_size=512, shuffle=True, num_workers=24,pin_memory=True)
...
for idx in range(rounds):
    ...
    if len(available_indices) < sample_size:
        print("Not enough available data to sample. Exiting loop.")
        break
    ...
    train_loader = DataLoader(train_dataset, batch_size=1024, shuffle=True, num_workers=24,pin_memory=True)
    train_model(model, train_loader, validation_loader, criterion, optimizer, device, epochs=100)
    
    current_accuracy, _, cm = evaluate_model_with_cm(model, test_loader, device, criterion, idx + 1, categories)
    # 테스트 단계에서 사용 예시
    num_classes = 7  # 실제 클래스 개수로 변경
    avg_sensitivity, avg_specificity, f1, auroc = calculate_metrics_multi(test_loader, model, num_classes)
    print(f'평균 민감도: {avg_sensitivity:.4f}')
    print(f'평균 특이도: {avg_specificity:.4f}')
    print(f'평균 F1-score: {f1:.4f}')
...
plt.bar(range(len(remaining_data_points)), remaining_data_points, tick_label=[f'{i+1}' for i in range(len(remaining_data_points))])
plt.xlabel('Round')
plt.ylabel('Certain Data')
plt.title('Certain Data Over Rounds')
plt.grid(True)
plt.savefig('/home/work/colonoscopy_data/dataset/active_learning_result/valyes/class7/Acc_all_split_active_learning_7class_valyes.png')"""

plt.figure(figsize=(13, 8))
plt.plot(accuracies, marker='o',label='Test Accuracies',color='orange')
plt.xlabel('Round')
plt.xticks(range(rounds + 1), ['0'] + [f'{i+1}' for i in range(rounds)], rotation=0)
plt.ylabel('Accuracy (%)')
plt.ylim([40, 100])
plt.grid(True)
plt.savefig('/home/work/colonoscopy_data/dataset/active_learning_result/valyes/class7/Acc_all_split_active_learning_7class_1025.png')
